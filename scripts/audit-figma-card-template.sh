...
#
# The snapshot is a structural readout of the canonical mainline _TEMPLATE
# page (UDS Components file `1XJoUJgtNpw4R0IIT3VjoK`, page id `7481:14`).
# The template is a designer-authored visual REFERENCE; the builder is the
# runtime source of truth. This audit is the bridge that keeps the two
# from silently diverging — exactly the failure mode that produced the
# 8-component drift documented in
# .cursor/skills/figma-component-card/references/mainline-rollout-drift-
# report.md.
#
# When the template changes:
#   1. Re-read the mainline _TEMPLATE page via use_figma.
#   2. Update .cursor/figma/state/card-template.snapshot.json.
#   3. Update build-card.js so the audit passes again.
#   4. Re-run any planned mainline rollout AFTER the recipe matches.
#
...
# Outer wrapper rounded — drift #3 from the mainline rollout report.
check_substring \
...
check_substring \
  ...
  "'Variant matrix'"
check_substring \
  "VARIANTS section description matches mainline _TEMPLATE" \
  "Each variant shows the full state matrix"
check_substring \
  "SUB-COMPONENTS section title is 'Component groups'" \
  "'Component groups'"
check_substring \
  "SUB-COMPONENTS section description matches mainline _TEMPLATE" \
  "Sub-component sets that compose with the primary set"
check_substring \
  "ANATOMY section description matches mainline _TEMPLATE" \
...
if ! grep -F "7481:14" "$SNAPSHOT" >/dev/null 2>&1; then
  echo "FAIL: snapshot does not reference the mainline _TEMPLATE page id 7481:14" >&2
  errors=$((errors + 1))
fi
...
if [ "$errors" -gt 0 ]; then
  ...
  echo "  mainline _TEMPLATE page expects (per .cursor/figma/state/card-template.snapshot.json)." >&2
  echo "" >&2
  echo "How to fix:" >&2
  ...
  echo "  - If the TEMPLATE actually changed: re-read the mainline _TEMPLATE page" >&2
  ...
  echo "" >&2
  echo "Background: .cursor/skills/figma-component-card/references/mainline-rollout-drift-report.md" >&2
  exit 1
fi

echo "OK — $checks check(s) passed. build-card.js matches card-template.snapshot.json (mainline _TEMPLATE page 7481:14)."
