...
export const SITE_CHANGELOG = [
    ...
    },
    {
      ...
      date: '2026-05-21',
      changes: [
        { type: 'changed', text: 'Design Language page (`uds-docs/docs/pages/design-language.html`) — replaced the academic "canon" and "ratification" language with natural design-system voice. Specifics: page lede rewritten ("How UDS components look in each state. Every preview on this page is a real component — the colors come from the same tokens as production. Where components disagree on the same state, both are shown so we can pick one."); hero meta row collapsed from three git-style stat pills (database / tune / code) into one friendly sentence; section 11 retitled "Naming guidance" (was "Naming canon"); section 12 retitled with a "needs review" badge (was "designer ratification"); every "Canonical pattern proposed" trailer reworded as "Suggested fix"; "in the canon today" → "in UDS today"; the "agent canon" cross-reference in the Related-pages footer reworded as "the same content for AI agents lives in..."; the "Cross-cutting" section heading reworded to "A few rules that apply across the board."' },
        ...
        { type: 'changed', text: 'Design Language page layout cleanup. (1) Removed the manual-style "01 / 02 / …" numeric prefixes from every section header — sections now just have a title. (2) Dropped the heavy `border-bottom` divider under section headers; spacing alone does the separation. (3) State-card tag (e.g. "rest", "pointer over") is no longer rendered in monospace — it reads as natural copy now. (4) Hid the leftover "LIVE PREVIEW" labels above each live preview (`.sg-dl-preview-label { display: none; }`) — the rendered component is self-evident, the label was visual noise. (5) "Cross-cutting" section retitled "A few rules that apply across the board" and dropped its `★` heading icon.' },
        { type: 'changed', text: 'SITE bumped 2026.05.21.6 → 2026.05.21.7. `app.js?v=` bumped 135 → 136 because `docs/data/site-changelog.js` changed (per `uds-site-changelog.mdc` — `app.js` imports `site-changelog.js`). Only the design-language page CSS + copy changed in this revision; `docs/app.js` itself did not change, so the bump is solely for the changelog cache-bust requirement.' }
      ]
    }
];
