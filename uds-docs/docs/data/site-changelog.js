...
export const SITE_CHANGELOG = [
    ...
    },
    {
      ...
      date: '2026-05-11',
      changes: [
        { type: 'removed', text: 'Reverted the Phase 16 + Phase 17 semantic-token bumps from the repo restructure (commits `6022fcd` and `6564ae8`). Those commits darkened ~38 tokens in `uds/tokens/semantic.css` — including the AnyoneHome (emerald) brand accents (emerald-70 → emerald-90) and the Inhabit (amber) brand accents (amber-70 → amber-90) — to clear axe-core WCAG AA contrast violations. The bumps were never sourced from Figma, were not authorized as a design change, and shifted visible brand identity. `semantic.css` is now restored bit-for-bit to the post-Figma-sync state from the UDS 0.3 release commit (`16e8e89`), so Figma and code agree again. Known trade-off: the contrast failures Phase 16/17 was masking are restored as a known issue (e.g. white text on `surface-interactive-default` is 3.45:1 in AnyoneHome, 2.66:1 in Inhabit, both below WCAG AA 4.5:1). The right fix is a design conversation — either accept those contrasts as documented exceptions, change the foreground token on the affected components, introduce a new `*-bold` surface variant for the WCAG-failing contexts only, or bump the tokens in Figma first and re-sync. The `scripts/audit-theme-contrast.sh` CI job will fail until that conversation lands; that is intentional, not a regression to fix in code.' },
        { type: 'changed', text: 'UDS 0.3 changelog `globalNotes` rewritten to reflect the revert. The previous \"Accessibility — bumped semantic color tokens…\" entry was the Phase 18 documentation for the unauthorized Phase 16/17 token shifts; it has been replaced with a `removed` entry describing the revert and an unchanged `fixed` entry that documents the surviving `button.css` cleanup (primitive-token references → semantic tokens in the `[data-btn-color=\"…\"]` overrides). `bash scripts/aggregate-changelog.sh` re-run to refresh `uds/CHANGELOG.json`.' }
      ]
    }
];
