...
export const SITE_CHANGELOG = [
    ...
    },
    {
      ...
      changes: [
        ...
        { type: 'changed', text: '`uds-docs/docs/pages/cursor-workflows.html` &mdash; new <code>uds-naming-conventions</code> card added to the Glob-scoped rules group, positioned immediately after the <code>uds-design-language</code> card since the two are paired (design-language covers <em>how things look</em> per state, naming-conventions covers <em>what we call them</em>; both load on any task). The card\'s body summarizes all ten categories the framework covers and explicitly calls out the companion relationship with <code>uds-design-language</code>. Group count chip bumped from 12 to 13.' },
        ...
        { type: 'changed', text: 'Merge with `main` resolution. While this branch was in flight, `main` shipped a 30+-commit sequence covering a complete Site Changelog page rewrite (renderers + data trim/rewrite passes 4a through 4m, PR #43, `2026.05.23.1`) plus the Cursor Workflows page redesign (Phase 4m.5, also `2026.05.23.1`) that restructured the rules section from tables to cards. The actual content changes don\'t conflict semantically &mdash; this branch adds a new rule; main restructured how rules are displayed; both branches agree the new rule should be documented. Mechanical conflicts were entirely on SITE bookkeeping plus the cursor-workflows page restructure: <code>version.txt</code>, <code>index.html</code> (SITE_VERSION inline + brand-bar text + <code>app.js?v=</code>), <code>site-changelog.js</code> (independent SITE bumps both landed on `2026.05.22.8` from the same `2026.05.22.7` base), and <code>cursor-workflows.html</code> (auto-merger left the old table additions stranded in a deleted region). Resolved by: (1) accepting main\'s `2026.05.23.1` for all SITE bookkeeping; (2) re-running `bump-site.sh` to land this branch at `2026.05.23.2`; (3) renumbering this branch\'s SITE_CHANGELOG entry from `2026.05.22.8` (originally written when this branch thought it would land on May 22) to `2026.05.23.2` with date `2026-05-23`; (4) dropping the old-table additions from cursor-workflows.html and adding the new rule as a card in main\'s new card-based Glob-scoped group, mirroring the `uds-design-language` card pattern; (5) regenerating `.cursor/TOOLCHAIN.md` post-merge; (6) bumping <code>app.js?v=</code> from main\'s `147` to `148` because <code>site-changelog.js</code> changed in this branch on top of main\'s change. The naming framework rule, the two skill updates, and the chosen card placement are all unchanged in substance &mdash; only the bookkeeping numbers and the cursor-workflows DOM shape were edited to fit on top of main.' },
        { type: 'changed', text: 'SITE bumped 2026.05.23.1 &rarr; 2026.05.23.2. `app.js?v=` bumped 147 &rarr; 148 because `docs/data/site-changelog.js` changed on top of main\'s change. Other cache-bust params unchanged &mdash; `site.css?v=`, `uds.css?v=`, `uds.js?v=`, and `demo-builder/index.js?v=` all stay where they are because no underlying file changed; the new rule landed in `.cursor/` and the cursor-workflows page is cache-busted via the runtime fetch helper rather than a `?v=` query.' }
      ]
    }
];
