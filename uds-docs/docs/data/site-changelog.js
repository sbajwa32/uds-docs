...
export const SITE_CHANGELOG = [
    ...
    },
    {
      ...
      date: '2026-05-21',
      changes: [
        { type: 'changed', text: 'Contrast Checker (`#/contrast-checker`) redesigned end-to-end into a focused, designer-first tool. Previous layout was a stacked stack of three same-weight sections (long WCAG explanation, 18 dense recommended-pairing cards, native-`<select>` picker, full matrix), which forced designers to read paragraphs before doing anything and gave no clear answer to the question "does this pairing work across themes?" New layout is hero-first with three zones, in order of importance: (1) hero Compare panel with a real-size live preview, big ratio + verdict, and a per-theme strip; (2) a tabbed Browse mode for breadth; (3) a collapsed `<details>` block for the WCAG/threshold reference.' },
        { type: 'added', text: 'Hero Compare panel — `.cc-hero` with two slot pickers (foreground + surface) and a swap button. Each picker is a custom popover (`.cc-popover`) with a search input, kind chip filters, scrollable grouped token list, click-outside-to-close, and Escape-to-close. The picker trigger shows a 24×24 swatch and the token short-name (mono). On the inside of the panel, a large `.cc-hero-stage` renders the actual surface filled-in: a paragraph of body text, an inline icon row (3 Material Symbols at body size), and a bordered "input" mock-up — so designers can see if a pair is readable at real reading scale, not just a 36×28 swatch. The verdict pill is large and centered with the contrast ratio in big monospace beside it.' },
        { type: 'added', text: 'Per-theme strip — `.cc-themes` row inside the hero shows the *same* foreground×surface pair evaluated across all 6 supported themes (Base Light, Base Dark, ResMan Light, ResMan Dark, AnyoneHome Light, Inhabit Light) simultaneously. Computed via a hidden detached-from-`<html>` probe element with each theme\'s `data-color-scheme` / `data-theme` attributes applied; reads the resolved CSS-variable RGB through `getComputedStyle()`, computes WCAG contrast, and renders a verdict mini-pill + ratio for each theme. Click any theme cell to flip the global theme bar to that combination — answers "does this pair work everywhere?" without making the designer manually flip themes.' },
        { type: 'added', text: 'Browse mode — `.cc-browse` segmented `udc-tabs`-style toggle between "Common pairings" (the curated semantic combinations, now rendered as compact 1-line list items with surface swatch + foreground sample + ratio + verdict, click → loads into Compare) and "Full matrix" (cleaner grid with sticky first column + header row, kind-tinted column accents, click any cell to load that pair into Compare). Above both views: kind chip filters (text / icon / border, multi-select) and a "Show only failing" toggle that hides anything that passes the kind\'s threshold. The same chip + fail-only filter applies to both views.' },
        { type: 'added', text: 'Click-to-pin from Browse to Compare — every curated row, every matrix cell, and every per-theme cell is interactive. Clicking populates the Compare panel without scrolling away (the page auto-scrolls to the hero). Compare panel state persists across browse-tab switches.' },
        ...
        { type: 'removed', text: 'Three same-weight `.sg-subsection` blocks ("Recommended pairings" / "Pick a surface or foreground" / "Full matrix") collapsed into the single hero+browse layout. The intro\'s 4-line wall-of-text paragraph dropped — the page now leads with the tool itself.' },
        { type: 'changed', text: '`docs/modules/contrast-checker/index.js` rewritten end-to-end. Color math (`resolveTokenRgb`, `parseRgb`, `srgbToLinear`, `luminance`, `contrastRatio`, `classifyVerdict`, `verdictLabel`, `formatRatio`, `isTransparent`, `isDisabledName`, `isNoneName`) preserved verbatim — same WCAG SC 1.4.3 / 1.4.11 thresholds. New code: `resolveTokenInTheme()` (per-theme rgb resolution via attached probe), `THEME_PROFILES` array (6 entries), `renderHero()` / `renderHeroStage()` / `renderThemeStrip()`, `renderBrowse()`, `openPopover()` / `closePopover()` / `renderPopoverList()` (custom searchable picker), `applyThemeProfile()` (click-theme-cell → flip global theme bar), `getActiveThemeProfileId()` (highlight currently-active theme strip cell). The MutationObserver on `<html>` data-attributes is unchanged — flipping the theme bar still recomputes everything.' },
        { type: 'changed', text: '`docs/pages/tools/contrast-checker.html` rewritten to match the new layout: title + 1-sentence subtitle, then `<div id="cc-app">` (the hero + browse + WCAG-help reference), with all internal markup populated by the JS module on first navigation.' },
        { type: 'changed', text: '`docs/site.css` `.cc-*` block (lines 1576-2007 in the previous build) replaced with the new layout rules: `.cc-app`, `.cc-hero`, `.cc-pickers`, `.cc-picker-trigger`, `.cc-popover`, `.cc-popover-search`, `.cc-popover-kinds`, `.cc-popover-list`, `.cc-popover-group`, `.cc-popover-option`, `.cc-swap-btn`, `.cc-hero-stage`, `.cc-stage-text`, `.cc-stage-icons`, `.cc-stage-input`, `.cc-verdict-row`, `.cc-themes`, `.cc-theme-cell`, `.cc-browse`, `.cc-browse-tabs`, `.cc-browse-filters`, `.cc-curated-list`, `.cc-curated-row`, `.cc-matrix-*` (cleaner header / cell). Verdict pill colors unchanged (success/info/warning/error subtle). Existing `.cc-sample`, `.cc-ratio`, `.cc-verdict-*`, `.cc-by-design` utility classes preserved verbatim — they\'re reused in the new layout.' },
        { type: 'changed', text: 'Lazy-import in `docs/app.js` `registerPageLoadHook("contrast-checker", ...)` now imports `./modules/contrast-checker/index.js?v=2` (cache-bust query) so the new module is fetched on first paint instead of stale previously-cached bytes — important because the dynamic `import()` is the only entry point for this module and there\'s no `<script src="...?v=N">` tag in `index.html` to bump.' },
        { type: 'changed', text: 'SITE bumped 2026.05.21.7 → 2026.05.21.8. `app.js?v=` bumped 136 → 137 (`app.js` itself changed for the dynamic-import cache-bust + `site-changelog.js` import). `site.css?v=` bumped 9 → 10 (`site.css` changed). `uds.css?v=`, `uds.js?v=`, and `demo-builder/index.js?v=` unchanged (no underlying file changed).' }
      ]
    }
];
