...
export const SITE_CHANGELOG = [
    ...
    {
      version: 'SITE 2026.05.11.2',
      date: '2026-05-11',
      changes: [
        { type: 'changed', text: 'Stale-reference cleanup pass across `uds-docs/` infra: the `bump-site.sh` final reminder now points at `docs/data/site-changelog.js` instead of `app.js` (matches the post-Phase-3c extraction); `release.sh` header comment now names `udsResolve()` (the actual helper in `docs/helpers/uds-path.js`) instead of the never-merged `udsPath()`; `uds-design-system.mdc` (the consumer-facing Cursor rule served from this repo) now points at `uds/components/<component>/spec.json` and `uds/components.json` instead of the long-removed `content/<component>.json`, and its component-count summary is updated from 21 → 29 (the live count comes from `uds/components.json`); `ai-context.json` likewise now advertises `uds/components/<component>/spec.json`, `uds/components.json`, and `uds/schemas/spec.schema.json` as the spec sources of truth, and tracks UDS version 0.3 instead of 0.2. None of these files are loaded into the SPA; updates are doc-only, so no asset cache-bust required.' },
