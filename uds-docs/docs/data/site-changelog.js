...
export const SITE_CHANGELOG = [
    ...
    {
      version: 'SITE 2026.05.20.3',
      date: '2026-05-20',
      changes: [
        { type: 'added', text: 'Scaffolded three new placeholder components in the date/time picker family: `calendar`, `time-picker`, `date-time-picker`. All three start at `placeholder` lifecycle status, with `since: "0.3"`, an empty `<id>.css`, and a single deferred-only example file. Spec JSON contains description / whenToUse / whenNotToUse / acceptanceCriteria / dependencies / single keyboard entry (Tab) / one `knownIssues` entry stating "No Figma component set exists yet" — exactly the same shape as the existing `date-picker` placeholder, so all four components present consistently as a family. `figmaNodeId` and `figmaPageNodeId` left null (no Figma reference yet; the designer can populate via the `link-figma-nodes` skill once Figma pages exist). Per-component `changelog.json` carries one `added` entry for 0.3 noting the placeholder status. `examples/manifest.json` declares `demoWeight: 0` so the placeholders are excluded from Demo Builder until real examples land. Sidebar links inserted into "Forms & Actions" — Calendar between Combobox and Date Picker, Time Picker and Date Time Picker right after Date Picker — to group the family together. Four `<div data-page="<id>">` blocks added before `</main>` in `index.html` (Calendar before the existing Date Picker block, then Time Picker and Date Time Picker after it). The existing `date-picker` placeholder was left untouched (`uds-source-of-truth.mdc` — modifying it would require an authorized Figma-sync path or explicit user direction; the user prompt scopes to drafting the new set). `uds-docs/uds/uds.css` regenerated via `migrate_component.regenerate_uds_orchestrators()` — now imports 32 component stylesheets (was 29). `uds-docs/uds/components.json` regenerated via `scripts/aggregate-components.sh` — now lists 32 components ordered by since/id. `uds-docs/uds/CHANGELOG.json` regenerated via `scripts/aggregate-changelog.sh` — 0.3 release now has 30 components / 56 entries (was 27 / 53). Audits pass cleanly: `audit-component-completeness.sh` (all 32), `audit-placeholders.sh`, `audit-token-usage.sh` (empty CSS is allowed). `audit-css-api-table.sh` skips the new components because no Code-tab `<table class="sg-api-table">` exists in their `data-page` blocks (deferred until Figma component set lands).' },
        { type: 'changed', text: 'SITE bumped 2026.05.20.2 → 2026.05.20.3. `uds.css?v=` bumped 57 → 58 because `uds-docs/uds/uds.css` gained three `@import` lines (calendar, time-picker, date-time-picker). `app.js?v=` bumped 129 → 130 because `docs/data/site-changelog.js` changed (`uds-site-changelog.mdc` cache-bust requirement: any change to a file imported by `app.js` requires the `app.js?v=` bump).' }
      ]
    }
];
