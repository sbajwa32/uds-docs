...
| `all` or no input given | Audit every component listed in `uds-docs/uds/components.json` (each component's spec lives at `uds-docs/uds/components/<id>/spec.json`) |
...
Read `uds-docs/uds/schemas/spec.schema.json` to get the field list. The "applicable" fields for completeness scoring (matching the `COMPLETENESS_FIELDS` constant exported from `uds-docs/docs/data/completeness-fields.js`) are:
...
_Audited at <timestamp>. Source of truth: `uds-docs/uds/components/<id>/spec.json` (Figma-derived; this audit is read-only — never edits the spec — per `uds-source-of-truth.mdc`)._
...
- `uds-docs/docs/data/completeness-fields.js` `COMPLETENESS_FIELDS` constant — the live source of the 22-field list (imported by `docs/app.js`)
