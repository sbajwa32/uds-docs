...
#### Content match against `uds-docs/content/<id>.json`

- [ ] HEADER title equals `json.title`
- [ ] HEADER description equals `json.description`
- [ ] HEADER css chip equals `json.dependencies.css[0]`
- [ ] USAGE "When to use" body equals `json.whenToUse`
- [ ] USAGE "When not to use" body equals `json.whenNotToUse`
- [ ] ACCESSIBILITY keyboard table rows match `json.accessibility.keyboard`
- [ ] META `DESIGNER` value equals `json.owner.designer`
- [ ] META `DEVELOPER` value equals `json.owner.developer`

If a JSON field is empty AND the corresponding card section was omitted, the check passes. If the JSON field is populated AND the card text doesn't match, FAIL.
...
- [`.cursor/agents/spec-audit.md`](./spec-audit.md) — sibling audit for JSON spec completeness
- [`uds-content-schema.mdc`](../rules/uds-content-schema.mdc) — JSON spec rules
